    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "team" and request._game_obj_:
            kwargs["queryset"] = Team.objects.filter(
                league__id=request._game_obj_.league.id
            )
        else:
            kwargs["queryset"] = Team.objects.filter()
        return super(GameTeamsInline, self).formfield_for_foreignkey(
...
class RegistrationsAdmin(admin.ModelAdmin):
        ...
        if db_field.name == "baggage" and request._registration_obj_:
            ...
            kwargs["queryset"] = (
                Baggage.objects.filter(
                    Q(registrations__league=request._registration_obj_.league)
                    | Q(registrations__id__isnull=True)
                )
                .order_by("id")
                .distinct()
            )
        return super(RegistrationsAdmin, self).formfield_for_foreignkey(
            db_field, request, **kwargs
        )
    ...
    def paypal_details(self, obj):
        paypal_row = (
            PayPalIPN.objects.filter(invoice=obj.paypal_invoice_id)
...
class TeamMemberModelChoiceField(forms.ModelChoiceField):
    def label_from_instance(self, team_member):
        ...
        return "{}, {} ({})".format(
            team_member.last_name, team_member.first_name, team_member.email
        )
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "user":
            ...
            if request._team_obj_:
                registration_user_ids = Registrations.objects.filter(
                    league__id=request._team_obj_.league.id
                ).values_list("user", flat=True)
                kwargs["queryset"] = (
                    get_user_model()
                    .objects.filter(id__in=registration_user_ids)
                    .order_by("last_name", "email")
                )
            else:
                kwargs["queryset"] = (
                    get_user_model().objects.filter().order_by("last_name", "email")
                )

            return db_field.formfield(**kwargs)

        return super(TeamMemberInline, self).formfield_for_foreignkey(
            db_field, request, **kwargs
        )
