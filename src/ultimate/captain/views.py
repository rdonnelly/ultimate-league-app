...
from ultimate.captain.models import (
    GameReport,
    GameReportAttendance,
    GameReportComment,
    GameReportScore,
)
from ultimate.forms import (
    EditTeamInformationForm,
    GameReportCommentForm,
    GameReportScoreForm,
    PlayerSurveyForm,
)
...
def index(request):
    ...
    # get teams where user is a captain and either the team is not hidden or there is a game in the past
    game_exists = Game.objects.filter(
        gameteams__team=OuterRef("id"), date__lte=timezone.now().date()
    )

    captain_teams = (
        Team.objects.annotate(game_exists=Exists(game_exists))
        .filter(
            Q(teammember__user=request.user),
            Q(teammember__captain=True),
            Q(hidden=False) | Q(game_exists=True),
        )
        .order_by("-league__league_start_date")
    )

    return render(request, "captain/index.html", {"captain_teams": captain_teams})
...
def editteam(request, team_id):
    ...
    if not bool(
        team.teammember_set.filter(user=request.user, captain=True)[0:1].count()
    ):
        raise Http403
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse("captaineditteam", kwargs={"team_id": team.id})
            )
        else:
    ...
    return render(request, "captain/editteam.html", {"team": team, "form": form})
    ...
    for team_member in team_members:
        ...
        writer.writerow(
            [
                ...
                getattr(
                    Registrations.objects.get(
                        user=team_member.user, league=team.league
                    ),
                    "attendance",
                    0,
                ),
...
def playersurvey(request, team_id):
    ...
    team_member_users = (
        get_user_model()
        .objects.filter(teammember__team=team)
        .exclude(id=request.user.id)
        .extra(
            select={
                "average_experience": "SELECT COALESCE(AVG(user_playerratings.experience), 0) FROM user_playerratings WHERE user_playerratings.user_id = user_user.id AND user_playerratings.experience != 0"
            }
        )
        .extra(
            select={
                "average_strategy": "SELECT COALESCE(AVG(user_playerratings.strategy), 0) FROM user_playerratings WHERE user_playerratings.user_id = user_user.id AND user_playerratings.strategy != 0"
            }
        )
        .extra(
            select={
                "average_throwing": "SELECT COALESCE(AVG(user_playerratings.throwing), 0) FROM user_playerratings WHERE user_playerratings.user_id = user_user.id AND user_playerratings.throwing != 0"
            }
        )
        .extra(
            select={
                "average_athleticism": "SELECT COALESCE(AVG(user_playerratings.athleticism), 0) FROM user_playerratings WHERE user_playerratings.user_id = user_user.id AND user_playerratings.athleticism != 0"
            }
        )
        .extra(
            select={
                "average_spirit": "SELECT COALESCE(AVG(user_playerratings.spirit), 0) FROM user_playerratings WHERE user_playerratings.user_id = user_user.id AND user_playerratings.spirit != 0"
            }
        )
        .distinct()
    )
    ...
    try:
        ratings_report, created = PlayerRatingsReport.objects.get_or_create(
        ...
        ratings_report = PlayerRatingsReport.objects.get(
            submitted_by=request.user, team=team
        )
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            for rating_data in formset.cleaned_data:
                ...
                ratings_row, created = PlayerRatings.objects.get_or_create(
                    ratings_report=ratings_report, user=data["user"], defaults=data
                )
                if not created:
            ...
            return HttpResponseRedirect(
                reverse("playersurvey", kwargs={"team_id": team_id})
            )
        else:
            messages.error(request, "There was an error on the form you submitted.")
        ...
        for team_member_user in team_member_users:
            try:
                last_rating = model_to_dict(
                    team_member_user.playerratings_set.filter(submitted_by=request.user)
                    .order_by("-updated")[0:1]
                    .get()
                )
    ...
    for i, form in enumerate(formset.forms):
        survey.append({"user": team_member_users[i], "form": form})

    return render(
        request,
        "captain/playersurvey.html",
        {
            "formset": formset,
            "ratings_choices": {
...
def gamereport(request, team_id, game_id):
    ...
    if not bool(
        ...
        game.gameteams_set.filter(
            team__teammember__user=request.user, team__teammember__captain=True
        )[0:1].count()
    ):
    ...
    try:
        ...
        try:
            game_report_comment = GameReportComment.objects.get(
                report=game_report, submitted_by=request.user
            )
    ...
    attendance = []
    score_formset_factory = modelformset_factory(
        GameReportScore, form=GameReportScoreForm, extra=2, max_num=2
    )
    ...
    if request.method == "POST":
        ...
        if timezone.now().date() < game.date:
            ...
            messages.error(
                request, "You cannot submit a game report before the game date."
            )
            ...
            if game_report:
                ...
                game_report = GameReport(
                    game=game, team=team, last_updated_by=request.user
                )
            game_report.save()
            ...
            for user_id in attendance:
                attendance_record = GameReportAttendance(
                    report=game_report, user_id=user_id
                )
                attendance_record.save()

            messages.success(request, "Your game report was updated successfully.")
            return HttpResponseRedirect(
                reverse("gamereport", kwargs={"game_id": game_id, "team_id": team_id})
            )

    else:
        comment_form = GameReportCommentForm(instance=game_report_comment)
        if game_report:
            for attendance_record in GameReportAttendance.objects.filter(
                report=game_report
            ):
                attendance.append(attendance_record.user.id)
            score_formset = score_formset_factory(
                queryset=GameReportScore.objects.filter(report=game_report)
            )
        else:
            ...
            score_formset = score_formset_factory(
                queryset=GameReportScore.objects.none(),
